...
import MyScreenDelegate from './MyScreenDelegate';

export default function MyScreen({ delegate }: { delegate: MyScreenDelegate }) {
    const txtCusIdRef = useRef<HTMLInputElement | null>(null);
    const txtCusNmRef = useRef<HTMLInputElement | null>(null);

    const handleSave = () => {


        console.log('txtCusIdRef', txtCusIdRef.current);

        console.log('txtCusIdRef', txtCusIdRef.current);
        console.log('txtCusNmRef', txtCusNmRef.current);

        const cusId = txtCusIdRef.current?.value ?? '';
        const cusNm = txtCusNmRef.current?.value ?? '';
        delegate.onSave(cusId, cusNm);
    };
